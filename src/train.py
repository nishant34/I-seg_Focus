...
if __name__ == '__main__':
    ...
    mri_data_train = H5Dataset("/content/drive/My Drive/train_target/data_train", mode='train')
    trainloader = dataloader.DataLoader(mri_data_train, batch_size=4, shuffle=True)
    mri_data_val = H5Dataset("/content/drive/My Drive/train_target/data_val", mode='val')
    ...
    for epoch in range (num_epoch):
        ...
        if (epoch % step_size_S) == 0 or epoch == (num_epoch - 1) or (epoch % 1000) == 0:
            torch.save(model_S.state_dict(), '/content/drive/My Drive/savedstate' + '%s_%s.pth' % (str(epoch).zfill(5), checkpoint_name))
