...
class FCN1(nn.Module):
    def __init__(self,num_input_features=32,drop_rate=0,num_classes=4):
        ...
        self.conv4 = _residual_block(num_input_features=128, num_output_features=256,is_first=True) 

        self.up_5 = nn.ConvTranspose3d(256, 128, kernel_size=4, stride=2 , padding=1, groups=num_classes, bias=False)
        self.conv5 = _residual_block(num_input_features=256, num_output_features=128,is_first=True)
        ...
        self.block3 = _residual_block(num_input_features=128, num_output_features=128,is_first=True)
        self.block30 = _residual_block(num_input_features=128, num_output_features=128,is_first=False)

        self.se3 = squeeze_excite_block(num_input_features=128)
        ...
        self.up2_5 = nn.ConvTranspose3d(128, 128, kernel_size=4, stride=2 , padding=1, groups=num_classes, bias=False)
        self.conv2_5 = _residual_block(num_input_features=256, num_output_features=128,is_first=True)
        ...
        self.bn_class = nn.BatchNorm3d(4)
    ...
    def forward(self,x):
        ...
        fse3 = self.se3(out1)
        out1 = self.gate3(fconv5)
        out1 = torch.mul(fse3,out1)
        out1=self.se3(out1)
        out1 = self.block30(out1)
        fblock3b = out1
        ...
        out1 = torch.cat([fblock3b,out1],1)
